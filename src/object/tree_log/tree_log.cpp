...
TreeLog::TreeLog() {
    ...
    if (!this->texture) {
        this->texture = std::make_unique<ppgso::Texture>(ppgso::image::loadBMP("textures/tree_log.bmp"));
    }
    if (!mesh) {
        mesh = std::make_unique<ppgso::Mesh>("models/tree_log.obj");
    }
}

bool TreeLog::update(Scene &scene, float dt) {
    generateModelMatrix();
    return true;
}

void TreeLog::render(Scene &scene) {
