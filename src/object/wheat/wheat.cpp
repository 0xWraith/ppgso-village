...
Wheat::Wheat() {
    ...
    if (!this->texture) {
        this->texture = std::make_unique<ppgso::Texture>(ppgso::image::loadBMP("textures/wheat.bmp"));
    }
    if (!mesh) {
        mesh = std::make_unique<ppgso::Mesh>("models/wheat.obj");
    }
}

bool Wheat::update(Scene &scene, float dt) {
    generateModelMatrix();
    return true;
}

void Wheat::render(Scene &scene) {
