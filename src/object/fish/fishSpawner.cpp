...
//        scene.sceneStructure->addChild(fish);
        auto fish = std::make_unique<Fish>(FISH_OBJECT_MODELS[randomFish], FISH_OBJECT_TEXTURES[randomFish], glm::vec3 {0.0, 5.0, 100.0}, glm::vec3 {glm::radians(45.0), 0, 0}, Fish::ENUM_FISH_TYPE::BUCKET_FISH);
        fish->scale = FISH_OBJECT_SCALES[randomFish];
...
//
        auto fish = std::make_unique<Fish>(FISH_OBJECT_MODELS[randomFish], FISH_OBJECT_TEXTURES[randomFish], position, glm::vec3 {glm::radians(45.0), randomAngle, 0}, Fish::ENUM_FISH_TYPE::WATER_FISH);
        fish->scale = FISH_OBJECT_SCALES[randomFish];
        scene.objects.push_back(move(fish));
