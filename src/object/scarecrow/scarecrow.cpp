...
Scarecrow::Scarecrow() {
    ...
    if (!this->texture) {
        this->texture = std::make_unique<ppgso::Texture>(ppgso::image::loadBMP("textures/scarecrow.bmp"));
    }
    if (!mesh) {
        mesh = std::make_unique<ppgso::Mesh>("models/scarecrow.obj");
    }
}

bool Scarecrow::update(Scene &scene, float dt) {
    generateModelMatrix();
    return true;
}

void Scarecrow::render(Scene &scene) {
