...
Pickaxe::Pickaxe() {
    ...
    if (!this->texture) {
        this->texture = std::make_unique<ppgso::Texture>(ppgso::image::loadBMP("textures/pickaxe.bmp"));
    }
    if (!mesh) {
        mesh = std::make_unique<ppgso::Mesh>("models/pickaxe.obj");
    }
}

bool Pickaxe::update(Scene &scene, float dt) {
    generateModelMatrix();
    return true;
}

void Pickaxe::render(Scene &scene) {
